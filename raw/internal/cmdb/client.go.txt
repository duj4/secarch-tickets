...
// PrepareConfig validates CMDB settings and applies defaults.
func PrepareConfig(cfg Config) (Config, error) {
	if cfg.TicketAPIURL == "" {
		return Config{}, fmt.Errorf("ticket_api_url is empty")
	}
	cfg.TicketBrowseURL = strings.TrimRight(strings.TrimSpace(cfg.TicketBrowseURL), "/")
	if cfg.TicketBrowseURL == "" {
		return Config{}, fmt.Errorf("ticket_browse_url is empty")
	}
	ticketBrowseURL, err := url.Parse(cfg.TicketBrowseURL)
	if err != nil || ticketBrowseURL.Scheme != "https" || ticketBrowseURL.Host == "" {
		return Config{}, fmt.Errorf("ticket_browse_url must be an absolute HTTPS URL")
	}
...
func NewClient(cfg Config, proxy httpclient.ProxyFunc) (*Client, error) {
